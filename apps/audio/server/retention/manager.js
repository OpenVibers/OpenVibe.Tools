...
// ═══════════════════════════════════════════════════════════════
// Audio.OpenVibe — Ephemeral Retention Manager
...
/**
 * Save a processed output file (already on disk from ffmpeg) to the output directory.
 * @param {string} sourcePath - Path to the ffmpeg output file
 * @param {string} ext - File extension (e.g. 'mp3', 'wav')
 ...
 */
function saveOutputFromFile(sourcePath, ext, mime, isAuthed = false, originalName = '') {
    ensureDirs();
    ...
    try {
        ...
        try { fs.unlinkSync(sourcePath); } catch { /* ok */ }
    }
...
module.exports = { saveOutput, saveOutputFromFile, getFile, removeFile, cleanup, getStats, startCleanup, stopCleanup };
