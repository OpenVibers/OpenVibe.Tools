...
/**
 * One file in 'file' or up to 5 in 'files' (the job endpoint takes either shape; merge sends 'files').
    ...
    upload.fields([{ name: 'file', maxCount: 1 }, { name: 'files', maxCount: 5 }])(req, res, (err) => {
        if (err) {
            ...
            if (err.code === 'LIMIT_FILE_COUNT' || err.code === 'LIMIT_UNEXPECTED_FILE') {
                return res.status(400).json({ error: 'Send one file in "file" or up to 5 in "files".' });
            }
            return res.status(400).json({ error: err.message });
        }
        const n = req.files ? Object.values(req.files).flat().length : 0;
        if (!n) return res.status(400).json({ error: 'No file uploaded. Send a file in the "file" field.' });
        next();
    });
}
